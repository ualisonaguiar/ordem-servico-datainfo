...
namespace InepZend\Cache\Storage\Adapter;
    ...
    public function addServers($id, $servers)
    {
        ...
        if ($resource instanceof MemcacheResource) {
            ...
            $resource['servers'] = array_merge(
                    $resource['servers'], array_udiff($servers, $resource['servers'], array($this, 'compareServers'))
            );
        }
        ...
        if (is_array($server)) {
            if (isset($server[0])) {
                ...
                $server = array_combine(
                        array_slice(array_keys($sTmp), 0, count($server)), $server
                );
            }
