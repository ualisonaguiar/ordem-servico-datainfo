...
namespace InepZend\Doctrine\ORM\Mapping\Driver;
...
/**
 * The DatabaseDriver reverse engineers the mapping metadata from a database custom Inep.
 */
class DatabaseDriver implements MappingDriver
{
    ...
    private function reverseEngineerMappingFromDatabase()
    {
        ...
        $arrTables = $this->getTableViewSchema();
        $this->tables = $this->manyToManyTables = $this->classToTableNames = array();

        foreach ($arrTables as $strTableName => $table) {
            ...
            if ($pkColumns == $allForeignKeyColumns && count($foreignKeys) == 2) {
                $this->manyToManyTables[$strTableName] = $table;
            } else {
                // lower-casing is necessary because of Oracle Uppercase Tablenames,
                // assumption is lower-case + underscore separated.
                $className = $this->getClassNameForTable($strTableName);

                $this->tables[$strTableName] = $table;
                $this->classToTableNames[$className] = $strTableName;
            }
        }
    }
    ...
    /**
     * Metodo responsavel pelo retorno das tables e views do schema
     *
     * @return array
     */
    private function getTableViewSchema()
    {
        $arrTable = array();
        foreach ($this->_sm->listTableNames() as $tableName) {
            try {
                $arrTable[$tableName] = $this->_sm->listTableDetails($tableName);
            } catch (\Exception $exception) {
                print_r("\nTabela {$tableName} nao gerada. Erro: {$exception->getMessage()} \n");
                continue;
            }
        }
        foreach ($this->_sm->listViews() as $viewName) {
            try {
                $arrTable[$viewName->getName()] = $this->_sm->listTableDetails($viewName->getName());
            } catch (\Exception $exception) {
                print_r("\nView {$viewName->getName()} nao gerada. Erro: {$exception->getMessage()} \n");
                continue;
            }
        }
        return $arrTable;
